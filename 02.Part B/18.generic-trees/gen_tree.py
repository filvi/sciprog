...
class GenericTree:
    ...
    def insert_child(self, new_child):
        ...
        if not isinstance(new_child, GenericTree):
            new_child = GenericTree(new_child)
        else:
            if self.has_child():
                backup = self._child
                self._child = new_child
                new_child._sibling = backup
                new_child._parent = backup._parent 
            else:
                self._child = new_child
                new_child._parent = self
# python -m unittest gen_tree_test.InsertChildTest

    def insert_children(self, new_children):
        for el in reversed(new_children):
            self.insert_child(el)
    ...
    def insert_sibling(self, new_sibling):
        ...
        """
        if self.is_root():
            raise Exception("roooooot😁")
        else:
            if self._sibling != None:
                backup = self._sibling
                self._sibling = new_sibling
                new_sibling._sibling = backup
                new_sibling._parent = backup._parent
            else:
                self._sibling = new_sibling
                new_sibling._parent = self.parent()

#    python -m unittest gen_tree_test.InsertSiblingTest  ##
    def insert_siblings(self, new_siblings):
        ...
        """
        for el in reversed(new_siblings):
            self.insert_sibling(el)
    ...
    def detach_child(self):
        ...
        if self.child() == None:
            ...
            if self.child().sibling()  != None:
                backup = self.child().sibling()
                self._child._sibling = None
...
#python -m unittest gen_tree_test.DetachChildTest
    ...
    def detach(self, data):
        ...
        while len(stack)>0:
            my = stack.pop()
            if my.child() != None:
                if my.child()._data == data:
                    if my.child().sibling() != None:
                        backup =  my.child().sibling()
                        my._child._parent = None
                        my._child._sibling = None
                        my._child = backup
                        return
                    else:
                        my._child._parent = None
                    ...
                    if my.child()!= None:
                        stack.append(my.child())
                    sib = my.child().sibling()
                    while sib != None:
                        if sib.child() != None:
                            stack.append(sib.child())
                        sib = sib.sibling()
